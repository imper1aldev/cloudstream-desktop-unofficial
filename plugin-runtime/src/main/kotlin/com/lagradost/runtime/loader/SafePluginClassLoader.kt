...
class SafePluginClassLoader(parent: ClassLoader) : ClassLoader(parent) {
    ...
    private fun isBlocked(name: String): Boolean {
        // Block all file system access completely
        if (name.startsWith("java.io.") || name.startsWith("java.nio.")) {
            // Allow some benign exceptions if really needed, but generally block all java.io
            // The plugin shouldn't need java.io classes as nicehttp returns strings/streams that are managed
            // Actually, nicehttp uses okio/InputStream. InputStream and IOException are in java.io
            // We must whitelist benign java.io classes used by OkHttp and normal operations:
            val safeIo = setOf(
                "java.io.InputStream",
