...
class CloudflareKiller : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response = runBlocking {
        ...
        if (!resolvingHosts.add(host)) {
            println("$TAG: Already resolving $host, skipping duplicate")
        ...
        println("$TAG: Failed to bypass Cloudflare for $host (will not retry this session)")
    ...
    private suspend fun bypassCloudflare(request: Request): Response? {
        ...
        println("$TAG: Loading Playwright to solve Cloudflare for $host")
        ...
        if (cookieHeader != null && cookieHeader.contains("cf_clearance")) {
            ...
            println("$TAG: Successfully bypassed Cloudflare for $host")
